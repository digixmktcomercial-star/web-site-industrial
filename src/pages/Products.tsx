      ...
      <section className="relative h-[400px] w-full overflow-hidden mb-12">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex flex-col justify-center relative z-10">
          <AnimatePresence mode="wait">
            ...
            >
              <h2 className="text-blue-400 text-xs font-black uppercase tracking-[0.3em]">
                {category === 'Todos' ? 'Clorosol Portugal' : category}
              </h2>
              <h1 className="text-5xl md:text-6xl font-black text-white tracking-tighter leading-none">
                {CATEGORY_BANNERS[category].title}
              </h1>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center space-x-4">
            <div className="relative group">
              ...
              <input
                ...
                className="pl-12 pr-6 py-4 bg-white border border-slate-200 rounded-2xl w-full md:w-80 shadow-sm focus:ring-4 focus:ring-blue-500/10 focus:border-blue-600 outline-none transition-all text-slate-700 font-medium"
              />
            </div>
            <button 
              onClick={() => setShowFilters(!showFilters)}
              className={`p-4 rounded-2xl border transition-all flex items-center space-x-2 font-bold text-sm ${
          ...
          <div className="flex items-center space-x-2">
            <button 
              ...
              className={`p-3 rounded-xl transition-all ${viewMode === 'grid' ? 'text-blue-600 bg-blue-50 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
            >
              <LayoutGrid className="w-5 h-5" />
            </button>
            <button 
              onClick={() => setViewMode('list')}
              className={`p-3 rounded-xl transition-all ${viewMode === 'list' ? 'text-blue-600 bg-blue-50 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
            >
